...
host as SPM, here's a trick to secure things a bit better. By creating a custom Docker network,
...
Then add the following to the `docker-compose.yml` file for both SPM and any other
...
Now in the SPM UI you can create a proxy host with `portainer` as the hostname,
...
The easy fix is to add a Docker environment variable to the Starlane Proxy Manager stack:
...
By default, SPM fetches IP ranges from CloudFront and Cloudflare during application startup. In environments with limited internet access or to speed up container startup, this fetch can be disabled:
...
SPM has the ability to include different custom configuration snippets in different places.
...
By default, SPM rotates the access- and error logs weekly and keeps 4 and 10 log files respectively.
