...
function AuthProvider({ children, tokenRefreshInterval = 5 * 60 * 1000 }: Props) {
	...
	// Handle OIDC token/error from URL query params (after callback redirect)
	useEffect(() => {
		const params = new URLSearchParams(window.location.search);
		const oidcToken = params.get("oidc_token");
		const oidcExpires = params.get("oidc_expires");
		const oidcErr = params.get("oidc_error");

		if (oidcToken && oidcExpires) {
			AuthStore.set({ token: oidcToken, expires: oidcExpires as unknown as number });
			setAuthenticated(true);
			// Clean URL params
			window.history.replaceState({}, document.title, window.location.pathname);
		} else if (oidcErr) {
			setOidcError(decodeURIComponent(oidcErr));
			window.history.replaceState({}, document.title, window.location.pathname);
		}
	}, []);
