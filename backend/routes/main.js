...
router.get("/docs", (req, res) => res.redirect(301, "/api/docs/"));
router.use(
	"/docs",
	swaggerUi.serve,
	async (req, res, next) => {
		try {
			...
			const origin = req.headers.origin || `${proto}://${req.hostname}`;
			spec.servers = [{ url: `${origin}/api` }];
			return swaggerUi.setup(spec, {
				customSiteTitle: "StarlaneProxyManager API Docs",
			})(req, res, next);
		} catch (err) {
			next(err);
		}
