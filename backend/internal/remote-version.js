...
// Tracks upstream Nginx Proxy Manager releases. SPM's own update check will be added in a future phase.
...
const internalRemoteVersion = {
	...
	get: async () => {
		...
		const version = pjson.npmBaseVersion.split("-").shift().split(".");
	...
	fetchUrl: (url) => {
		...
		const headers = {
			"User-Agent": `StarlaneProxyManager v${pjson.version}`,
		};
